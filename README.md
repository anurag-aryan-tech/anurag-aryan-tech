<!-- PROFILE README -->

# 👋 Hey there! Welcome to my Coding Journey 🚀  

<div align="center">

  <img src="logos/avatar.png" alt="Avatar" height="200" width="200">

  <!-- 🔹 Typing animation -->
  <img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=500&size=22&duration=2800&pause=1000&color=00BFFF&center=true&vCenter=true&width=650&lines=Code.+Build.+Learn.+Repeat.;Aspiring+Data+Scientist.;Python+%7C+DSA+%7C+Tkinter+%7C+CustomTkinter" alt="Typing SVG" />

</div>

---

## 🧠 About Me  

💬 **Actively seeking constructive feedback to improve my code**  
🚀 **Passionate about clean, maintainable architecture & problem-solving**  
🎯 **Currently focused on DSA, Data Science, and GUI development with Python**  
🤝 **Open to collaborations, discussions, and idea exchanges**

> 💡 *“Every expert was once a beginner. The difference? They didn’t quit.”*

---

## 🧰 Tech Stack & Skills  

<div align="center">

### 🐍 Core Language  
<img src="https://skillicons.dev/icons?i=python" height="60" />

### 🖥️ GUI Development (Hands-on)
<img src="logos/tkinter.png" height="60" />
&nbsp;
<img src="logos/customtkinter.png" width="60" height="60" />

### 🔭 Data Science (Learning & Practicing)
<img src="https://logo.svgcdn.com/l/numpy.png" width="60" height="60" />
&nbsp;
<img src="https://logo.svgcdn.com/devicon/pandas-original.png" width="60" height="60" />
&nbsp;
<img src="https://logo.svgcdn.com/logos/jupyter.png" width="60" height="60" />
&nbsp;
<img src="https://colab.research.google.com/img/colab_favicon_256px.png" width="60" height="60" />
&nbsp;
<img src="https://upload.wikimedia.org/wikipedia/commons/7/7c/Kaggle_logo.png" width="100" height="50" />

### 🧩 Tools & Platforms  
<img src="https://skillicons.dev/icons?i=git,github,vscode,postman" height="60" />

</div>

---

## 📚 Learning Resources I Recommend  

### 🐍 Python & DSA  
- 🧩 **LeetCode** — Structured DSA problem-solving  
- 📘 **GeeksforGeeks** — Strong conceptual explanations  
- 💡 **HackerRank** — Quick challenges & fundamentals  

### 🧭 Growth & Engineering Mindset  
- 🎯 **Roadmap.sh** — Clear learning paths  
- 💻 **freeCodeCamp** — Hands-on, project-based learning  
- 📖 **Official Python Docs** — The ultimate reference  

---

## 🏆 GitHub Stats  

<div align="center">

  <img src="https://streak-stats.demolab.com/?user=anurag-aryan-tech&theme=tokyonight&hide_border=true" height="150"/>

</div>

---

## 🧭 Current Focus  

- 🔥 **Building real-world Python projects** (GUI + logic-heavy)  
- 🧩 **Deepening DSA fundamentals** — quality over quantity  
- 🧠 **Learning Data Science step-by-step**  
- 📆 **Consistency mantra:** Code • Learn • Build • Improve  

---

<br/><br/>
  <div align="center">
    <img src="https://media.tenor.com/qJ5evVs-_uUAAAAC/coding.gif" width="350" alt="Developer Working GIF" />
    <br/><br/>
    <p style="font-size: 18px;">
      <strong>Last Updated: 2025</strong><br>
      <em>Built with passion, patience, and plenty of ☕</em>
    </p>

  </div>

</div>